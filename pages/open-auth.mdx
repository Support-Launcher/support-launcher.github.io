...
import { DynamicVersion } from "components/dynamic-version";

<DynamicVersion version="1.1.4">
Avec gradle :
...
<Callout type="info" emoji="✅">
  ...
  **$VERSION**. Allez vous sur la page des
...
<Callout type="warning" emoji="⚠️">
  Il faut choisir **une seule** méthode d'auth parmi les suivantes, faites
  attention a ne pas implémenter 2 de ces méthodes, cela serait inutile.
</Callout>
...
<Callout type="warning" emoji="⚠️">
  Attention, cette méthode n'est pas une méthode Async, c'est a dire que votre
  programme ne tournera plus en arrière plan lorsque cette fenêtre sera ouverte.
  Cela peut causer des problèmes, par exemple, votre interface graphique ne
  tournera plus et vous aurez un beau message de windows vous disant que le
  programme ne réponds plus. Cette méthode peux quand même être utile dans
  certains cas, c'est pourquoi je la présente ici. Si vous voulez éviter ces
  problèmes, utiliser la fonction loginWithAsyncWebview() (voir plus bas)
</Callout>
...
- L' _acces token_ est utilisé pour lancer le jeu, par exemple avec la librairie [OpenLauncherLib](https://github.com/Litarvan/OpenLauncherLib)
